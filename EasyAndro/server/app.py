...
import os, threading, subprocess, shlex, time, pexpect, shutil, logging, traceback, importlib.util, urllib.request, re
...
def set_adb_proxy(proxy_ip, proxy_port, flutter_yn = False):
    ...
    proxy_string = f"{proxy_ip}:{proxy_port}"
    if proxy_string == ":0":
        add_log("✅ ADB proxy disabled")
    elif not PROXY_RE.fullmatch(proxy_string):
        add_log(f"❌ Invalid proxy format: {proxy_string}")
...
def pull_apk():
    ...
    if not PACKAGE_RE.fullmatch(selectedPackage):
        add_log("❌ Invalid package name")
        raise ValueError("Invalid package name")

    if len(selectedPackage) > 255:
        add_log("❌ Invalid package name")
        raise ValueError("Invalid package name")
